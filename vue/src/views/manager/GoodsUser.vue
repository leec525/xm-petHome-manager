          ...
          <el-col :span="6" v-for="item in tableData" :key="item.id" style="margin-bottom: 20px">
            <div style="display: flex">
              <div style="flex: 1; padding-right: 5px">
                <img :src="item.img" alt="" style="width: 100%; height: 155px; border-radius: 10%; border: 1px solid #cccccc">
              </div>
              <div style="flex: 1; padding-left: 5px">
                <div style="font-size: 15px; font-weight: 550; color: #817a70">{{item.name}}</div>
                <div style="margin-top:5px; font-size: 20px; font-weight: 550; color: red">￥ {{item.price}}</div>
                <div style="margin-top:5px; font-size: 13px; color: #4b4949">剩余数量：{{item.num}}</div>
                <div style="margin-top: 10px">
                  <el-input-number v-model="item.tmpNum" @change="handleChange" :min="1" :max="item.num" size="mini"></el-input-number>
                </div>
