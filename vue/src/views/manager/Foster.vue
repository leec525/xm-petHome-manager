<template>
  <div>
    <div class="search">
      <el-select placeholder="请选择寄养状态" style="width: 200px" v-model="status">
      ...
      <el-table :data="tableData" stripe  @selection-change="handleSelectionChange">
        ...
        <el-table-column label="操作" width="180" align="center">
          <template v-slot="scope">
            <el-button plain type="primary" v-if="user.role === 'USER' && scope.row.status === '待确认'" size="mini" @click="handleEdit(scope.row)">编辑</el-button>
            ...
            <el-button plain type="primary" v-if="user.role === 'ADMIN' && scope.row.status === '寄养中'" size="mini" @click="roomReset(scope.row)">领回</el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-dialog title="宠物信息" :visible.sync="fromVisible" width="40%" :close-on-click-modal="false" destroy-on-close>
    ...
    <el-dialog title="房间信息" :visible.sync="roomVisible" width="40%" :close-on-click-modal="false" destroy-on-close>
      <el-form label-width="100px" style="padding-right: 50px" :model="form">
        <el-form-item prop="roomId" label="选择房间号">
...
export default {
  ...
  methods: {
    ...
    roomInit(row){   //分配房间调用的方法
      this.form = JSON.parse(JSON.stringify(row))   // 给form对象赋值  注意要深拷贝数据
      this.roomVisible = true  //打开弹窗
    },
    roomReset(row){  //宠物领回调用的方法
      this.form = JSON.parse(JSON.stringify(row))   // 给form对象赋值  注意要深拷贝数据
